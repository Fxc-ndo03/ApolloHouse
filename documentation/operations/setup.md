...
- Secrets required by voice/search providers (for example Gemini)
...
Fill `.dev.vars` with at least `DEVICE_SHARED_SECRET` and `GEMINI_API_KEY`, and leave `MOCK_VOICE=1` — it skips real STT/TTS *and* vector recall locally, so a dev session costs nothing in ElevenLabs credits or embedding calls.

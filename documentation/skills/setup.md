...
2. **Which keys do they already have?** Gemini, ElevenLabs, Tavily, Resend, a GitHub App. Map answers to the secrets table below; missing optional keys are fine.
...
| Key | Missing → | Class |
|---|---|---|
| `GEMINI_API_KEY` | Every real turn fails — STT, reasoning, embeddings have no guard in the turn path | **Hard-fail** (unless `MOCK_VOICE=1`) |
...
2. **`provision`** — idempotently creates: R2 buckets `apollo-media` + `apollo-media-preview` (dev uses the preview bucket), Vectorize index `apollo-memory` with `--dimensions=768 --metric=cosine`, queue `apollo-jobs`. The dimension count is **pinned to `GEMINI_EMBEDDING_MODEL` = `text-embedding-004`** (768-dim vectors). This coupling fails **silently** if broken: a wrong-dims index rejects every insert and query, memory writes vanish and recall returns nothing, but turns keep working — Apollo just never remembers. Success: all four resources exist (rerunning is a no-op).

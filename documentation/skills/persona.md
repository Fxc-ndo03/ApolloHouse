...
3. **STT language** — `src/voice/stt.ts`, `transcribeAudioWithGemini`, the request body line `language: input.languageCode ?? 'es'`. Change the `'es'` default (Gemini language code, e.g. `'en'`, `'pt'`).
...
## 7. OpenRouter removal note

All references to `OPENROUTER_API_KEY`, `OPENROUTER_MODEL`, `OPENROUTER_STT_MODEL`, `OPENROUTER_RESEARCH_MODEL`, `OPENROUTER_CODING_MODEL`, `OPENROUTER_EMBEDDING_MODEL` have been removed from the codebase. The sole AI provider is now Google Gemini via `GEMINI_API_KEY`, `GEMINI_MODEL`, `GEMINI_LIVE_MODEL`, `GEMINI_EMBEDDING_MODEL`. Update any external documentation or scripts accordingly.

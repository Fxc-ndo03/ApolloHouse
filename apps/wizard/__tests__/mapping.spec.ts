...
describe('describeGeminiKeyStatus', () => {
  it('reports available models when present', () => {
    expect(
      describeGeminiKeyStatus({
        models: [{ name: 'models/gemini-1.5-flash' }, { name: 'models/gemini-1.5-pro' }],
      }),
    ).toBe('2 models available');
  });

  it('returns generic valid message for empty models', () => {
    expect(describeGeminiKeyStatus({ models: [] })).toBe('key valid');
  });

  it('returns undefined for an unexpected payload', () => {
    expect(describeGeminiKeyStatus({ unexpected: true })).toBeUndefined();
    expect(describeGeminiKeyStatus({})).toBeUndefined();
  });
});

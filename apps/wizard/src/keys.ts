...
const geminiKeyResponseSchema = z.object({
  models: z.array(z.object({ name: z.string() })).optional(),
});

export function describeGeminiKeyStatus(rawResponse: unknown): string | undefined {
  const parsedResponse = geminiKeyResponseSchema.safeParse(rawResponse);
  if (!parsedResponse.success) {
    return undefined;
  }
  const models = parsedResponse.data.models;
...
export async function validateGeminiApiKey(apiKey: string): Promise<KeyValidationResult> {
  try {
    ...
    return { isValid: true, successDetail: describeGeminiKeyStatus(responseBody) };
  } catch (error) {
    return {
      isValid: false,

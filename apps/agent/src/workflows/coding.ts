      ...
      const agentOutcome = await step.do(
        ...
        async () => {
          ...
          if (this.env.CODING_ENGINE !== 'legacy' && proxyOrigin !== undefined) {
            const geminiApiKeyForProxy = this.env.GEMINI_API_KEY ?? '';
            const proxySigningKey =
              geminiApiKeyForProxy.length > 0
                ? geminiApiKeyForProxy
                : this.env.GEMINI_API_KEY;
            const codingModelId =
              geminiApiKeyForProxy.length > 0
                ? (this.env.GEMINI_MODEL ?? 'models/gemini-3.6-flash')
      ...
      if (!extraction.hasChanges) {
        ...
        await step.do('persist-log', async () => {
          ...
          await apollo.writeStorageObject({
            key: documentKey,
            content: redactSecretsFromText(runLog),

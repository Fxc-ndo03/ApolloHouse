...
import { PcBridge } from './pc-bridge';
import {
...
import { queryMemoryVectors } from '@/memory/vector';
...
import { TTS_STREAM_CHUNK_BYTE_LENGTH } from '@/voice/wav';
import { TTS_PCM_SAMPLE_RATE_HZ, TTS_PCM_CHANNEL_COUNT } from '@/voice/elevenlabs';
...
export type ApolloState = {
  ...
  readonly responseOutputTarget: 'pc' | 'server';
};
...
export class Apollo extends Agent<Env, ApolloState> {
  ...
  // Rate limiter for Gemini 3.6 Flash (5 RPM = 1 req per 12s)
  #geminiRateLimiter = {
    tokens: 5,
    lastRefill: Date.now(),
    maxTokens: 5,
    refillIntervalMs: 12000, // 12 seconds = 5 RPM
    queue: [] as Array<() => void>,
    processing: false,
  };
  ...
  #storageBucket: StorageBucket | null = null;

  get storageBucket(): StorageBucket {
    if (this.#storageBucket === null) {
      this.#storageBucket = createStorageBucket(
        this.ctx.storage.sql as unknown as SqlStorage,
      );
    }
  ...
  async onStart(): Promise<void> {
    ...
    // Warmup alarm: prevent DO hibernation by touching storage every 5 minutes
    await this.scheduleEvery(300, 'warmupKeepAlive');
  }

  async warmupKeepAlive(): Promise<void> {
    // Lightweight storage touch to prevent hibernation
    await this.ctx.storage.sql.exec(`SELECT 1`);
  }

  // ========================================================================
  // Rate Limiter for Gemini 3.6 Flash (5 RPM free tier)
  // ========================================================================

  async #refillRateLimiterTokens(): Promise<void> {
    const now = Date.now();
    const elapsed = now - this.#geminiRateLimiter.lastRefill;
    const tokensToAdd = Math.floor(elapsed / this.#geminiRateLimiter.refillIntervalMs);
    if (tokensToAdd > 0) {
      this.#geminiRateLimiter.tokens = Math.min(
        this.#geminiRateLimiter.maxTokens,
        this.#geminiRateLimiter.tokens + tokensToAdd,
      );
      this.#geminiRateLimiter.lastRefill = now;
    }
  }

  async #processRateLimiterQueue(): Promise<void> {
    if (
      this.#geminiRateLimiter.processing ||
      this.#geminiRateLimiter.queue.length === 0
    ) {
      return;
    }
    this.#geminiRateLimiter.processing = true;

    while (this.#geminiRateLimiter.queue.length > 0) {
      await this.#refillRateLimiterTokens();
      if (this.#geminiRateLimiter.tokens >= 1) {
        this.#geminiRateLimiter.tokens -= 1;
        const resolve = this.#geminiRateLimiter.queue.shift();
        if (resolve) resolve();
      } else {
        // Wait for next token refill
        const waitMs =
          this.#geminiRateLimiter.refillIntervalMs -
          (Date.now() - this.#geminiRateLimiter.lastRefill);
        await new Promise((r) => setTimeout(r, Math.max(0, waitMs)));
      }
    }
    this.#geminiRateLimiter.processing = false;
  }

  async acquireGeminiToken(): Promise<void> {
    await this.#refillRateLimiterTokens();
    if (this.#geminiRateLimiter.tokens >= 1) {
      this.#geminiRateLimiter.tokens -= 1;
      return;
    ...
    await new Promise<void>((resolve) => {
      this.#geminiRateLimiter.queue.push(resolve);
    });
    // Process queue (this will wait for token)
    await this.#processRateLimiterQueue();
  }

  getGeminiRateLimiterStatus(): { tokens: number; queueLength: number } {
    return {
      tokens: this.#geminiRateLimiter.tokens,
      queueLength: this.#geminiRateLimiter.queue.length,
    };
  }
  ...
  #getActiveDeviceConnection(): Connection | null {
    const connections = this.getConnections(DEVICE_CONNECTION_TAG);
  ...
  // Tool callback: ringAlarm (called by this.schedule when alarm time arrives)
  async ringAlarm(payload: { id: string; label: string | null }): Promise<void> {
    const connection = this.#getActiveDeviceConnection();

    if (!connection) {
      // S5 disconnected: retry in 15s (max 3 attempts)
      this.#alarmRetryCount = (this.#alarmRetryCount ?? 0) + 1;
      if ((this.#alarmRetryCount ?? 0) <= 3) {
        await this.schedule(new Date(Date.now() + 15_000), 'ringAlarm', payload);
      }
      return;
    }

    this.#alarmRetryCount = 0;
    this.#activeAlarmId = payload.id;
    this.#alarmStopSignal = false;

    // 1. Cambia UI state
    this.#applyUiEvent('ALARM_RING');
    connection.send(
      encodeServerToDeviceMessage({
        type: 'alarm_ring',
        id: payload.id,
    ...
    // 2. Arranca loop de audio de timbre
    await this.#streamAlarmSoundLoop(connection, payload.id);
  }

  async #streamAlarmSoundLoop(connection: Connection, alarmId: string): Promise<void> {
    const alarmSound = await this.#loadAlarmSound();
    if (!alarmSound) {
      console.error('No se pudo cargar sonido de alarma');
      return;
    }
    ...
    connection.send(
      encodeServerToDeviceMessage({
        type: 'tts_start',
        format: 'pcm',
        bytes: alarmSound.byteLength,
        sampleRate: TTS_PCM_SAMPLE_RATE_HZ,
        channels: TTS_PCM_CHANNEL_COUNT,
      }),
    );

    const MAX_RING_MS = 5 * 60_000; // techo de seguridad: 5 min
    const startedAt = Date.now();

    // LOOP REAL: repite hasta dismiss, cambio de alarma, o timeout 5min
    while (
      !this.#alarmStopSignal &&
      this.#activeAlarmId === alarmId &&
      Date.now() - startedAt < MAX_RING_MS
    ) {
      await streamAudioChunksAtPlaybackPace({
        audioBuffer: alarmSound,
        sampleRateHz: TTS_PCM_SAMPLE_RATE_HZ,
        channelCount: TTS_PCM_CHANNEL_COUNT,
        chunkByteLength: TTS_STREAM_CHUNK_BYTE_LENGTH,
        wait: (ms) => new Promise((r) => setTimeout(r, ms)),
        shouldStop: () => this.#alarmStopSignal || this.#activeAlarmId !== alarmId,
        send: (chunk) => {
          if (this.#alarmStopSignal || this.#activeAlarmId !== alarmId) return;
          connection.send(chunk);
        },
      });

      if (!this.#alarmStopSignal && this.#activeAlarmId === alarmId) {
        await new Promise((r) => setTimeout(r, 400));
      }
    }

    // SIEMPRE cerrar el stream (dismiss, timeout, o cambio de alarma)
    connection.send(encodeServerToDeviceMessage({ type: 'tts_end' }));

    // Solo nulear si seguimos siendo dueños del ID (evita pisar alarma nueva)
    if (this.#activeAlarmId === alarmId) {
      this.#activeAlarmId = null;
    }
  }

  // Carga sonido (fallback tono programático)
  async #loadAlarmSound(): Promise<ArrayBuffer | null> {
    // TODO: Cuando subas el .pcm a R2, descomenta:
    // const obj = await this.storageBucket.get('alarm-sound.pcm');
    // if (obj) return await obj.arrayBuffer();

    // Fallback: tono programático (La 880Hz, 1.5s)
    return this.#generateSimpleTone(880, 1500);
  }

  // Genera tono senoidal simple (PCM 24kHz mono 16-bit little-endian)
  #generateSimpleTone(frequency: number, durationMs: number): ArrayBuffer {
    const sampleRate = TTS_PCM_SAMPLE_RATE_HZ;
    const samples = Math.floor((sampleRate * durationMs) / 1000);
    const buffer = new ArrayBuffer(samples * 2);
    const view = new DataView(buffer);

    for (let i = 0; i < samples; i++) {
      const t = i / TTS_PCM_SAMPLE_RATE_HZ;
      const attack = Math.min(1, i / (TTS_PCM_SAMPLE_RATE_HZ * 0.01));
      const release = Math.min(1, (samples - i) / (TTS_PCM_SAMPLE_RATE_HZ * 0.01));
      const envelope = attack * release;
      const value = Math.sin(2 * Math.PI * frequency * t) * envelope * 0.3;
  ...
  async onMessage(connection: Connection, message: WSMessage): Promise<void> {
    ...
    switch (deviceMessage.type) {
      ...
      case 'alarm_dismiss': {
        if (deviceMessage.id === this.#activeAlarmId) {
          this.#alarmStopSignal = true;
          this.#activeAlarmId = null;
          this.#pushUiState(connection);
        }
        break;
      }
      case 'set_response_output_target': {
        this.setState({
          ...this.state,
          responseOutputTarget: deviceMessage.target,
        });
        break;
      }
    }
  ...
  async getConsoleStatus(rawInput: ConsoleSecretInput): Promise<ConsoleStatusSnapshot> {
    ...
    return buildConsoleStatusSnapshot({
      ...
      responseOutputTarget: this.state.responseOutputTarget,
    });
  }
  ...
  async sendConsoleBroadcastText(
    ...
    const outcome = await deliverBroadcastText({
      ...
      mediaBucket: this.storageBucket,
  ...
  async listConsoleJobs(
    ...
    const input = consoleSecretInputSchema.parse(rawInput);
    await this.#assertDashboardSecret(input.secret);
    return listConsoleJobDocuments(
      asConsoleDocumentBucket(this.storageBucket),
      this.name ?? 'default',
    );
  }
  ...
  @callable()
  async writeStorageObject(rawInput: {
    key: string;
    content: string;
    contentType?: string;
  }): Promise<void> {
  ...
  @callable()
  async readFirmwareManifestRpc(): Promise<
    { version: string; key: string; changelog?: string } | undefined
  > {
    return readFirmwareManifest(this.storageBucket);
  }

  @callable()
  async readFirmwareObjectRpc(rawInput: {
    key: string;
  }): Promise<{ content: string; size: number } | null> {
    const input = z.object({ key: z.string().min(1) }).parse(rawInput);
    const obj = await this.storageBucket.get(input.key);
    if (obj === null) return null;
    const buffer = await obj.arrayBuffer();
    return { content: new TextDecoder().decode(buffer), size: obj.size };
  }
  ...
  @callable()
  async setResponseOutputTarget(target: 'pc' | 'server'): Promise<ApolloState> {
    this.setState({
      ...this.state,
      responseOutputTarget: target,
    });
    // Notify the device of the change if connected
    for (const connection of this.getConnections(DEVICE_CONNECTION_TAG)) {
      connection.send(
        encodeServerToDeviceMessage({
          type: 'set_response_output_target',
          target,
          ts: Date.now(),
        }),
      );
    ...
    try {
      ...
      const values =
        geminiApiKey.length > 0
          ? await embedTextWithGemini({
              geminiApiKey,
              modelId: this.env.GEMINI_EMBEDDING_MODEL ?? 'models/text-embedding-004',
              text: query,
            })
          : await (
              ...
              modelId: this.env.GEMINI_EMBEDDING_MODEL,
              text: query,
            });
        ...
        const chatResult =
          geminiApiKey.length > 0
            ? await chatWithGemini({
                ...
                messageList: buildThreadDigestMessageList(transcriptText) as unknown as {
                  role: string;
                  content: string | null;
                }[],
              })
  ...
  async #executeTurn(
    ...
    const deskToolEffects = createDeskToolEffects({
      ...
      scheduleAlarm: async ({ triggerAt, label }) => {
        const id = crypto.randomUUID();
        await this.schedule(triggerAt, 'ringAlarm', { id, label });
      },
    });
        ...
        connection,
        {
          ...
          getPlaybackAckForSequence: (sequence) =>
            this.#lastPlaybackAck !== null && this.#lastPlaybackAck.sequence === sequence
              ...
              : null,
          telemetrySnapshot: this.#lastTelemetrySnapshot,
          acquireGeminiToken: () => this.acquireGeminiToken(),
          pcVisualConnection: [...this.getConnections(PC_VISUAL_CONNECTION_TAG)][0],
        },
        turnPart,
      );
    } catch (error) {
  ...
  @callable()
  async debugListTtsModels(): Promise<
    readonly { name: string; displayName: string; supportedActions: readonly string[] }[]
  > {
    ...
    if (!response.ok) {
      ...
      throw new Error(`ListModels falló ${response.status}: ${detail.slice(0, 500)}`);
    }
    ...
    return models
      ...
      .map((m: any) => ({
        name: m.name,
        displayName: m.displayName,
        supportedActions: m.supportedActions ?? m.supportedGenerationMethods ?? [],
      }));
  }
}

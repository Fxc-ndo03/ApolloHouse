...
import type { GeminiChatMessage, GeminiChatResult, GeminiToolCall } from '@/voice/llm';
...
export function summarizeToolCallForTranscript(toolCall: GeminiToolCall): string {
...
async function requestWrapUpSummary(
  ...
  messageList: readonly GeminiChatMessage[],
): Promise<string> {
  try {

...
export const recallMemoryTool: ToolDefinition = {
  ...
  async handler(args, context) {
    ...
    try {
      const { queryMemoryVectors } = await import('@/memory/vector');
      const geminiApiKey = context.environment.GEMINI_API_KEY ?? '';
      const values =
        geminiApiKey.length > 0
          ? await (
              await import('@/voice/geminiEmbeddings')
            ).embedTextWithGemini({
              geminiApiKey,
              modelId:
                context.environment.GEMINI_EMBEDDING_MODEL ?? 'models/text-embedding-004',
          ...
          : await (
              ...
              modelId: context.environment.GEMINI_EMBEDDING_MODEL,
              text: parsedArgs.query,
            });

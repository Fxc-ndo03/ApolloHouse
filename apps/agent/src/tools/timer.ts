...
const setAlarmArgsSchema = z.object({
...
export const setAlarmTool: ToolDefinition = {
  name: 'set_alarm',
  safety: 'safe',
  description: 'Programa una alarma/despertador para una hora específica',
  parameters: {
    type: 'object',
    properties: {
      isoDateTime: {
        type: 'string',
        description: 'Fecha/hora ISO 8601 (ej: 2025-08-26T07:00:00-03:00)',
      },
  ...
  async handler(args, context) {
    ...
    const parsedArgs = setAlarmArgsSchema.parse(args);
    const triggerAt = new Date(parsedArgs.isoDateTime);
    if (isNaN(triggerAt.getTime())) {
      return { ok: false, summary: 'Fecha inválida' };
    }
    await context.effects.scheduleAlarm({
      triggerAt,
      label: parsedArgs.label ?? null,
    });
    return {
      ok: true,
      summary: `Alarma programada para ${triggerAt.toLocaleString('es-AR')}`,
    };
  },
};

...
function createFakeGeminiFetch(summaryText: string): typeof fetch {
...
describe('compactThreadMessageList', () => {
  ...
  it('summarizes the old stretch and keeps the recent tail out of it', async () => {
    ...
    const compactionResult = await compactThreadMessageList(
      environment,
      messageList,
      createFakeGeminiFetch('Hablaron del viaje a Salta y quedó pendiente el auto.'),
    );

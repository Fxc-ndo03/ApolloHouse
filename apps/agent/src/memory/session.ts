...
import type { GeminiChatMessage } from '@/voice/llm';
  ...
  const chatResult =
    geminiApiKey.length > 0
      ? await chatWithGemini({
          geminiApiKey,
          modelId: environment.GEMINI_MODEL ?? 'models/gemini-3.6-flash',
          fetchImplementation,
          messageList: [
            {
              role: 'system',
          ...
          ] as unknown as { role: string; content: string | null }[],
        })
      : await (
          await import('@/voice/llm')
        ).chatWithGemini({
          geminiApiKey: environment.GEMINI_API_KEY,
          modelId: environment.GEMINI_MODEL,
          fetchImplementation,
          messageList: [
            {
              role: 'system',
...
): readonly GeminiChatMessage[] {
  return messageList
    .map(mapSessionMessageToChatMessage)
    .filter((message): message is GeminiChatMessage => message !== undefined);
}

export async function buildRecentTurnHistoryMessageList(
  session: Session,
): Promise<readonly GeminiChatMessage[]> {

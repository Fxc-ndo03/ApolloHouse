...
// The subset of StorageBucket the broadcast path touches, kept structural so the
// spec can substitute an in-memory bucket.
export type BroadcastMediaBucket = StorageBucket;
...
async function speakBroadcastText(input: {
  ...
  const speechAudio = input.isMockVoice
    ...
    : await synthesizeApolloSpeech({
        storageBucket: input.mediaBucket,
